...
from config_management.data_config_muon_decay import DataConfig  # noqa: E402
...
def _load_kde_model(models_dir: Path) -> object:
    with (models_dir / "muon_decay_kde_wrapped.pkl").open("rb") as f:
        return dill.load(f)


def _load_density_scale_factor() -> float:
    p = _REPO_ROOT / "data/processed_data/muon_decay_scale_factor.txt"
    if p.exists():
        return float(np.loadtxt(p))
    return 1.0


def _load_sr_best(sr_results_path: Path, sr_index: int) -> tuple[callable, dict[str, object]]:
    import sympy as sp
...
def _compute_boundary_polygon(samples_xy: np.ndarray):
    if not DataConfig.domain_estimation:
        return None

    # Notebook uses Shapely buffer(-offset) on the convex hull polygon.
    # To avoid a hard dependency on Shapely in CLI runs, approximate this with:
    # 1) convex hull vertices
    # 2) shrink towards centroid by a factor (1 - offset)
    # 3) Delaunay-based point-in-hull test
    from scipy.spatial import ConvexHull, Delaunay

    hull = ConvexHull(samples_xy)
    hull_points = samples_xy[hull.vertices]
    centroid = np.mean(hull_points, axis=0)
    offset = float(DataConfig.domain_shrink_offset)
    shrink = max(0.0, 1.0 - offset)
    hull_points_shrunk = centroid + shrink * (hull_points - centroid)
    return Delaunay(hull_points_shrunk)


def _invalid_mask(points_xy: np.ndarray, boundary_triangulation) -> np.ndarray:
    if boundary_triangulation is None:
        return np.zeros(points_xy.shape[0], dtype=bool)

    inside = boundary_triangulation.find_simplex(points_xy) >= 0
    return ~inside


def _integrate_kde_on_square(kde_model: object, square: Square, n: int, boundary_triangulation) -> float:
    ...
    densities[_invalid_mask(pts, boundary_triangulation)] = 0.0
    return float(np.sum(densities) * dx * dy)


def _integrate_sr_on_square(
    sr_func, square: Square, n: int, density_scale_factor: float, boundary_triangulation
) -> float:
    pts, dx, dy = _make_square_grid(square, n)
    ...
    dens[_invalid_mask(pts, boundary_triangulation)] = 0.0
...
def _square_report(
    *,
    ...
    kde_mass = _integrate_kde_on_square(kde_model, square, n=n_integral, boundary_triangulation=boundary_triangulation)
    sr_mass = _integrate_sr_on_square(
        sr_func,
        square,
        n=n_integral,
        density_scale_factor=density_scale_factor,
        boundary_triangulation=boundary_triangulation,
    )

    n_integral_fine = int(n_integral) * 2
    kde_mass_fine = _integrate_kde_on_square(
        kde_model, square, n=n_integral_fine, boundary_triangulation=boundary_triangulation
    )
...
def _format_block(label: str, square: Square, results: dict[str, float]) -> str:
    ...
    return "\n".join(
        [
            f"{label}  (x∈[{square.x0:.3f},{square.x1:.3f}], y∈[{square.y0:.3f},{square.y1:.3f}])",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "using the muon-decay KDE wrapper + last SR expression (no normalizing flow)."
        )
    )
    parser.add_argument(
        "--samples",
        type=Path,
        default=None,
        help="Optional CSV of samples already in pipeline space (expects >=2 columns; uses first two).",
    )
    parser.add_argument(
        "--samples-raw",
        type=Path,
        default=Path("data/MuonDecay.csv"),
        help=(
            "Raw muon decay CSV (default: data/MuonDecay.csv). Reads DataConfig.columns and applies "
            "min-max scaling using the raw file's min/max."
        ),
    )
    parser.add_argument(
        "--models-dir",
        type=Path,
        default=Path("models"),
        help="Directory containing muon_decay_kde_wrapped.pkl (default: models).",
    )
    parser.add_argument(
        "--sr-results",
        type=Path,
        default=Path("data/pareto_results/muon_decay_results.py"),
        help="SR pareto results python file (default: data/pareto_results/muon_decay_results.py).",
    )
    parser.add_argument(
        "--sr-index",
        type=int,
        default=-1,
        help="Index into SR equation list (default: -1 for the last / best-loss equation).",
    )
    parser.add_argument(
        "--square-size",
        type=float,
    ...
    parser.add_argument(
        ...
        type=int,
        default=200,
        help="Grid resolution per axis for numeric integration (default: 200).",
    )
    ...
    sr_results_path = _resolve_repo_path(args.sr_results) or (_REPO_ROOT / "data/pareto_results/muon_decay_results.py")
    ...
    if samples_raw_path is not None:
        ...
        samples_label = str(samples_raw_path)
        ...
        samples_label = str(samples_path)

    boundary_triangulation = _compute_boundary_polygon(samples_xy)

    print(f"samples: {samples_label}  (N={samples_xy.shape[0]})")
    print(f"models: {models_dir}")
    print(
        f"sr_results: {sr_results_path}  "
        f"(sr_index={args.sr_index}, complexity={int(sr_meta['complexity'])}, loss={float(sr_meta['loss']):.6g})"
    )
    ...
    print(f"domain_mask: {'convex_hull_shrink' if boundary_triangulation is not None else 'none'}")
    print("")
    ...
    for idx, (name, square) in enumerate(_iter_squares(args.square_size)):
        ...
        if idx > 0:
            print("")
        print(_format_block(name, square, results))
